"""myolegtorso velocity env config: flat terrain, tendon effort, posture from keyframe."""
...
def myolegstorso_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """myolegtorso flat velocity: keyframe posture, randomized at reset; tendon effort action."""
  ...
  # Base velocity from entity state (no IMU in myolegtorso XML).
  ...
  # Foot height: use myolegtorso touch sites.
  ...
  # myolegtorso has no self_collision sensor; leave contact-dependent rewards zero.
  for name in [
    "foot_clearance",
    "foot_swing_height",
    "foot_slip",
